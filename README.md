...
开启 `save_leg_images: true` 后，插件会将以下类型的图片保存到 `leg_images/` 目录：

- **腿部露出** - 短裤、短裙露腿、丝袜/长筒袜、脚丫特写等
- **清秀特征** - 人物长相清秀柔和，像女孩子的男性或中性化人物
- **高浓度2次元卡通** - 动漫角色、虚拟角色、CG 绘画、插画等二次元风格图片

```
astrbot_plugin_cosplay_image_saver/
├── cosplay/          # 女装图片（排除卡通/二次元）
│   └── QID [QQ 号]/
└── leg_images/       # 灰色定义图片（腿部、2次元卡通等）
    └── QID [QQ 号]/
```

**处理流程**：
```
图片 → 女装检测 → 是（且非卡通）→ 保存到 cosplay/
              ↓ 否/是卡通
              → 灰色图片检测 → 是（含卡通）→ 保存到 leg_images/
                          ↓ 否
                          → 跳过
```

**注意**：高浓度2次元卡通图片（动漫角色、虚拟角色、表情包等）会自动从女装检测中排除，避免误识别卡通表情包为女装。如需保存卡通图片，请开启 `save_leg_images`。
